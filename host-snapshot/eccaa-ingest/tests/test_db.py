...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(eccaa_reports)")}
    assert {
        "case_id", "report_url", "pdf_url", "pdf_url_es", "pdf_url_en", "pdf_path",
        "title", "event_class", "aircraft", "registration", "date_of_occurrence",
        "location", "operator", "country", "narrative_text", "source_tier", "lang",
        "status", "discovered_at", "updated_at",
    } <= rcols

    # eccaa_accidents columns IDENTICAL to ciaiac_accidents
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(eccaa_accidents)")}
    assert {
...
def test_status_index_exists():
    ...
    assert "idx_eccaa_reports_status" in indexes
