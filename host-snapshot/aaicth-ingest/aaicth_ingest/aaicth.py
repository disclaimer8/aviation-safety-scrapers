...
SSL:
  ops.mot.go.th has a certificate chain issue → verify=False required.
"""
...
def make_client():
    """Return an httpx.Client configured for ops.mot.go.th (SSL verify=False)."""
    import httpx
    return httpx.Client(
        headers=HEADERS,
        follow_redirects=True,
        verify=False,  # ops.mot.go.th has cert chain issue
        timeout=60.0,
    )
