...
class TestDiscover:
    ...
    def test_interim_case_id_has_suffix(self):
        ...
        assert row is not None, "Interim row must have case_id with -interim suffix"
...
class TestBuild:
    ...
    def test_superseded_row_skipped(self):
        self._insert_parsed(
            ...
            "Interim narrative text for this case that is more than 80 chars long.",
    ...
    def test_site_slug_format(self):
        self._insert_parsed(
            "aaicth-3/2009", "final",
            "Full narrative text about the ATR72 accident at Samui airport - more than eighty characters long."
        )
        build(self.conn)
        row = self.conn.execute(
