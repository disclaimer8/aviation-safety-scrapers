...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "AAIIB-2023-RP-C229", narrative="N" * 200,
...
def test_build_skips_below_narrative_floor():
    conn = _conn()
    _seed_parsed(conn, "AAIIB-2023-RP-C1", narrative="X" * 79)
...
def test_build_country_is_ph():
    conn = _conn()
    _seed_parsed(conn, "AAIIB-2023-RP-C1174", narrative="N" * 200)
    pipeline.build(conn)
    assert conn.execute(
