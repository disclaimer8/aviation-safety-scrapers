...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(caav_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(caav_accidents)")}
    assert {
...
def test_country_default_vn():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO caav_accidents (case_id) VALUES ('VN-001')")
    row = conn.execute(
        "SELECT country FROM caav_accidents WHERE case_id='VN-001'"
    ).fetchone()
...
def test_status_index_exists():
    ...
    assert "idx_caav_reports_status" in indexes
