...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(taiib_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(taiib_accidents)")}
    assert {
...
def test_country_default_lv():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO taiib_accidents (case_id) VALUES ('yl-eva-2024-05-04')")
    row = conn.execute(
        "SELECT country FROM taiib_accidents WHERE case_id='yl-eva-2024-05-04'"
    ).fetchone()
    assert row["country"] == "LV"
...
def test_status_index_exists():
    ...
    assert "idx_taiib_reports_status" in indexes
