...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "m67", narrative="N" * 200, event_class="Accident",
...
def test_build_skips_below_floor():
    conn = _conn()
    _seed_parsed(conn, "x", narrative="X" * 79, event_class="Accident")
    assert pipeline.build(conn) == 0
    assert conn.execute("SELECT status FROM taiib_reports WHERE case_id='x'").fetchone()["status"] == db.STATUS_SKIPPED
...
def test_build_skips_scanned_tier():
    ...
    conn = _conn()
    _seed_parsed(conn, "x", narrative="X" * 200, event_class="Accident", source_tier="scanned")
    assert pipeline.build(conn) == 0
    assert conn.execute("SELECT status FROM taiib_reports WHERE case_id='x'").fetchone()["status"] == db.STATUS_SKIPPED
...
def test_build_country_is_lv():
    conn = _conn()
    _seed_parsed(conn, "x", narrative="N" * 200, event_class="Accident")
    pipeline.build(conn)
