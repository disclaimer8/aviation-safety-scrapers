...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(aaicnp_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(aaicnp_accidents)")}
    assert {
...
def test_country_default_np():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO aaicnp_accidents (case_id) VALUES ('AAIC-NP-1-2026')")
    row = conn.execute(
        "SELECT country FROM aaicnp_accidents WHERE case_id='AAIC-NP-1-2026'"
    ).fetchone()
    assert row["country"] == "NP"
...
def test_status_index_exists():
    ...
    assert "idx_aaicnp_reports_status" in indexes
