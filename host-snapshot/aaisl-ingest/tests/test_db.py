...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(aaisl_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(aaisl_accidents)")}
    assert {
...
def test_country_default_lk():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO aaisl_accidents (case_id) VALUES ('4R-CAE-20260107')")
    row = conn.execute(
        "SELECT country FROM aaisl_accidents WHERE case_id='4R-CAE-20260107'"
    ).fetchone()
    assert row["country"] == "LK"


def test_accidents_columns_match_ciaiac_shape():
    """aaisl_accidents must mirror ciaiac_accidents column-for-column."""
    conn = db.connect(":memory:")
    db.init_schema(conn)
    acols = [r["name"] for r in conn.execute("PRAGMA table_info(aaisl_accidents)")]
    assert acols == [
...
def test_status_index_exists():
    ...
    assert "idx_aaisl_reports_status" in indexes
