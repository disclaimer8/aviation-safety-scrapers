# gcaagy_ingest/cli.py
import argparse
import os

from . import httpc, gcaagy, db
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={
            "User-Agent": gcaagy.UA,
            "Referer": gcaagy.REFERER,
        },
        proxy=proxy,
        timeout=60,
        delay=gcaagy.DELAY,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="gcaagy-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="gcaagy.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("GCAAGY_PROXY"),
        help="HTTP/SOCKS proxy URL (or set GCAAGY_PROXY env var)",
    )
