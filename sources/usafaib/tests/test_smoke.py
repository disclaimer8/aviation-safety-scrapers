"""Smoke tests for usafaib-ingest.
...
the stated grounds that "usafaib_scraper.py runs its ingest at module scope, so
...
import usafaib_scraper

HERE = pathlib.Path(__file__).resolve().parent.parent
SCRIPT = HERE / "usafaib_scraper.py"
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert usafaib_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (usafaib_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "usafaib" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(usafaib_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == usafaib_scraper.__name__]
