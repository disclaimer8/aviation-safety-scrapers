...
def test_build_skips_below_narrative_floor():
    conn = _conn()
    _seed_parsed(conn, "4R-X-20240101", narrative="X" * (pipeline._NARRATIVE_FLOOR - 1), source_tier="short",
                 event_class="Incident")
    assert pipeline.build(conn) == 0
    assert conn.execute(
...
def test_build_country_is_lk():
    conn = _conn()
    _seed_parsed(conn, "4R-CAE-20260107", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident")
    pipeline.build(conn)
    assert conn.execute(
...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "LK-FOO", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident",
