...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=knkt.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="knkt-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover→fetch→build)",
    )
    ap.add_argument("--db", default="knkt.db")
    ...
    ap.add_argument(
        "--proxy",
        default=os.environ.get("KNKT_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $KNKT_PROXY env var)",
    )
    return ap
