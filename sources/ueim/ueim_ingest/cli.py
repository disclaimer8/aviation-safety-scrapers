...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=ueim.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="ueim-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover->fetch->build)",
    )
    ap.add_argument("--db", default="ueim.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the TR+EN listing pages are "
                         "always walked in full")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("UEIM_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $UEIM_PROXY env var)",
    )
    return ap
