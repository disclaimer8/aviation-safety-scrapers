...
the stated grounds that "daaina_scraper.py runs its ingest at module scope, so
...
import daaina_scraper

HERE = pathlib.Path(__file__).resolve().parent.parent
SCRIPT = HERE / "daaina_scraper.py"
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert daaina_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (daaina_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "daaina" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(daaina_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == daaina_scraper.__name__]
