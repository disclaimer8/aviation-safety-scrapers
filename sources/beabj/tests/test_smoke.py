...
the stated grounds that "beabj_scraper.py runs its ingest at module scope, so
...
import beabj_scraper

HERE = pathlib.Path(__file__).resolve().parent.parent
SCRIPT = HERE / "beabj_scraper.py"
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert beabj_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (beabj_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "beabj" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(beabj_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == beabj_scraper.__name__]
