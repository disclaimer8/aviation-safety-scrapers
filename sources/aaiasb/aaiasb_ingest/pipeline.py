...
# 300 matches prod's NARRATIVE_MIN and costs this source nothing: all 209
# rows in production are already over it, averaging 34,472 characters.
# _common/tests/test_narrative_floor enforces the value across the tree.
_NARRATIVE_FLOOR = 300  # chars; rows below this are non-text/scanned PDFs
...
def discover(conn):
    ...
    for start in aaiasb.STARTS:
        ...
        seen += len(rows)
        if not rows:
            # Per page this is only worth a note: the last page of a paginated
            # listing is often short or empty. The run-level guard is below.
    ...
    if seen == 0:
        # Across all four listing pages, not per page. 209 rows from this
        # source are already in production, so zero everywhere is the markup
        # changing rather than AAIASB publishing nothing — and returning 0
        # would be indistinguishable from a clean run.
        #
        # Placed after the loop and outside the try above: that try's
        # `except Exception` would swallow this and continue, which is how a
        # guard becomes decorative.
        raise RuntimeError(
            "[aaiasb discover] every listing page parsed to zero rows. The"
