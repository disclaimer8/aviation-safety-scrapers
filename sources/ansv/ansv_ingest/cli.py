...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="ansv-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="ansv.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("ANSV_PROXY"),
        help="HTTP/SOCKS proxy URL (or set ANSV_PROXY env var)",
    )
