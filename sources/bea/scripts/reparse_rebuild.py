...
Usage:
    python scripts/reparse_rebuild.py --db /path/to/bea.db [--dry-run]
...
Motivation:
  ...
  full 20K-60K-char French narratives.  Both bugs are fixed in bea_ingest
  >= this commit.  Running this script recovers ~2 209 previously-skipped
  rows on the mini-PC's existing bea.db without re-downloading PDFs.
"""
import argparse
import sys
import os
...
def main():
    ...
    parser.add_argument("--db", required=True, help="Path to bea.db SQLite file")
