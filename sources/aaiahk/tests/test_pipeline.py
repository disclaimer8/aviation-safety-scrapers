...
def test_build_creates_accident_row():
    conn = _conn()
    narr = "N" * (pipeline._NARRATIVE_FLOOR + 100)
...
def test_build_country_is_hk():
    conn = _conn()
    _seed_parsed(conn, "IVR-2025-01", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident")
    pipeline.build(conn)
    assert conn.execute(
...
def test_build_skips_below_narrative_floor():
    conn = _conn()
    _seed_parsed(conn, "IVR-2025-10", narrative="X" * (pipeline._NARRATIVE_FLOOR - 1), event_class="Incident")
    assert pipeline.build(conn) == 0
    assert conn.execute(
...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "PLR-2026-01", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident",
...
def test_build_report_type_from_event_class():
    conn = _conn()
    _seed_parsed(conn, "IVR-2025-02", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Serious incident")
    pipeline.build(conn)
    assert conn.execute(
...
def _seed_built_report(conn, case_id, *, superseded_by=None):
    ...
    conn.execute(
        ...
        (case_id, "2025-01-01", "Test Aircraft", None, None, "HKIA",
         "HK", "N" * (pipeline._NARRATIVE_FLOOR + 100), None, "https://example.com/report.pdf",
