...
- Each <tr> is one occurrence with columns:
  ...
  Preliminary and Investigation each hold zero or one link, but Interim
  holds zero to six: Annex 13 asks for a statement every 12 months while an
  investigation stays open, so a long-running case accumulates one per year
  (ITR-2026-06, ITR-2025-05, ITR-2024-03 are the same occurrence). Measured
  against the live page: 30 of 45 rows carry more than one interim.
  The site lists newest first and the first match wins, so the chosen
  interim is the most recent — verified against the live page, not just the
  pinned fixture.
