# aaiahk_ingest/cli.py
import argparse
import os

from . import httpc, aaiahk, db
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={
            "User-Agent": aaiahk.UA,
            "Referer": aaiahk.REFERER,
        },
        proxy=proxy,
        timeout=60,
        delay=aaiahk.DELAY,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="aaiahk-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aaiahk.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AAIAHK_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AAIAHK_PROXY env var)",
    )
