...
Notes:
  ...
  than _NARRATIVE_FLOOR (300 chars) are skipped.  country='HK'.
...
# 300 matches prod's NARRATIVE_MIN. It was 80 on the host, which admitted
# rows of 80-299 chars that prod can only ever render noindex — work done
# to produce a page nobody is meant to see. _common/tests/test_narrative_floor
# enforces the floor across the tree.
...
def discover(conn, client, full=False):
    ...
    if not rows:
        ...
        # 47 rows from this source are already in production, so an
        ...
        raise RuntimeError(
            ...
            "[aaiahk discover] listing parsed to zero rows. The markup has"
