...
def _make_client(proxy=None):
    ...
    return httpc.make_client(
        headers={"User-Agent": griaa.UA, "Referer": griaa.REFERER},
        proxy=proxy,
        timeout=60.0,
    )
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="griaa-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="griaa.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("GRIAA_PROXY"),
        help="HTTP/SOCKS proxy URL (or set GRIAA_PROXY env var)",
    )
