...
def test_extract_text_returns_stdout(monkeypatch):
    monkeypatch.setattr(pdf.subprocess, "run", lambda *a, **k: _Done(0, b"  Informe final texto  "))
    assert pdf.extract_text("x.pdf") == "Informe final texto"
