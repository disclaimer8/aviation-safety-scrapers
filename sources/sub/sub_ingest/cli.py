...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=sub.HEADERS, proxy=proxy, timeout=180)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="sub-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover->fetch->build)",
    )
    ap.add_argument("--db", default="sub.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the hub is always walked "
                         "in full")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("SUB_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $SUB_PROXY env var)",
    )
    return ap
