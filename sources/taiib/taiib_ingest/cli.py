...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="taiib-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="taiib.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("TAIIB_PROXY"),
        help="HTTP/SOCKS proxy URL (or set TAIIB_PROXY env var)",
    )
