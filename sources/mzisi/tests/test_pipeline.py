...
def test_parse_scanned_tier(monkeypatch):
    ...
    monkeypatch.setattr(pipeline, "extract_text", lambda p: "x" * (pipeline._NARRATIVE_FLOOR - 1))
    pipeline.parse(conn)
    assert conn.execute(
...
def test_build_falls_back_to_staging_case_id_when_no_source_event_id():
    ...
    _seed_parsed(
        ...
        narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Summary",
...
def test_build_skips_below_floor():
    conn = _conn()
    _seed_parsed(conn, "mzisi-2006-cam", narrative="x" * (pipeline._NARRATIVE_FLOOR - 1), event_class="Final report")
...
def test_build_country_is_si():
    conn = _conn()
    _seed_parsed(conn, "mzisi-2024-a", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Final report")
    pipeline.build(conn)
    assert conn.execute(
...
def test_build_mixed_rows():
    conn = _conn()
    long_n = "Z" * (pipeline._NARRATIVE_FLOOR + 100)
...
def test_parse_event_date_null_when_scanned(monkeypatch):
    conn = _conn()
    _seed_fetched(conn, "mzisi-2006-cam", pdf_path="cam.pdf")
    monkeypatch.setattr(pipeline, "extract_text", lambda p: "x" * (pipeline._NARRATIVE_FLOOR - 1))  # no dates
    pipeline.parse(conn)
    row = conn.execute(
        "SELECT date_of_occurrence, location, aircraft "
