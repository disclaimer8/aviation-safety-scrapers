...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="mzisi-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="mzisi.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("MZISI_PROXY"),
        help="HTTP/SOCKS proxy URL (or set MZISI_PROXY env var)",
    )
