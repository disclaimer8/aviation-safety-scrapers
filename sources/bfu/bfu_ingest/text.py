...
def make_site_slug(aircraft, registration, location):
    ...
    return f"crash-{base}" if base else "crash-aaib"
...
# BEA titles follow the pattern:
#   <EventClass> to the <Aircraft> (registered|identified) <Reg>
#   [operated by <Operator>] on <DD/MM/YY[YY]> (at|near|off|…) <Location>
#   [<trailing bracket>]
#
# The old single mega-regex failed on:
#   • 2-digit years  (YY instead of YYYY)
#   • "on the <location>" location prefix
#   • "en route" with no following place (trailing punctuation/bracket ate the rest)
#   • multi-aircraft titles (first aircraft + both regs in one blob)
# This piecewise approach extracts each field independently so one mismatch
...
def parse_event_title(title):
    """
    Piecewise parser for BEA event titles.
