...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=india.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="india-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover→fetch→build)",
    )
    ap.add_argument("--db", default="india.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the single index page is "
                         "always fully parsed")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("INDIA_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $INDIA_PROXY env var)",
    )
    return ap
