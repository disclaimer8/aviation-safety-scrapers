...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="aaiib-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aaiib.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AAIIB_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AAIIB_PROXY env var)",
    )
