...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=nsia.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="nsia-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover→fetch→build)",
    )
    ap.add_argument("--db", default="nsia.db")
    ...
    ap.add_argument(
        "--proxy",
        default=os.environ.get("NSIA_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $NSIA_PROXY env var)",
    )
    return ap
