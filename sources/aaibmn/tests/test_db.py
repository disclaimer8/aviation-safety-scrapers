...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(aaibmn_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(aaibmn_accidents)")}
...
def test_country_default_mn():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO aaibmn_accidents (case_id) VALUES ('x')")
    row = conn.execute("SELECT country FROM aaibmn_accidents WHERE case_id='x'").fetchone()
    assert row["country"] == "MN"
...
def test_status_index_exists():
    ...
    assert "idx_aaibmn_reports_status" in indexes
