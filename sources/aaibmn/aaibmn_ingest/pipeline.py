...
# 30, deliberately below prod's NARRATIVE_MIN of 300, and listed in
# _common/tests/test_narrative_floor's exception table with that reason.
#
# This source's older reports are scans with no text layer, so parse()
# falls back to the listing title — which carries the date, the aircraft
# type and the registration. Those fields are the occurrence; the narrative
# is not. At 300 the row is never built and the occurrence is lost from the
# database entirely, not merely rendered noindex.
