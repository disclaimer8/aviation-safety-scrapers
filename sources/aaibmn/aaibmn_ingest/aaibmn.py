...
def make_client(proxy=None, **_kw):
    ...
    The previous version built a transport only when a proxy was passed, so
    an ordinary run got httpx's default transport with retries=0 — the same
    bug the other packages carry a comment about having fixed, which never
    reached this one. httpc always builds the transport, and adds
    response-level retries, the robots gate and the SSRF guard.
    """
    from . import httpc
    return httpc.make_client(headers=HEADERS, proxy=proxy, timeout=60, delay=DELAY)
