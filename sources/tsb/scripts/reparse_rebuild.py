...
Usage:
    python scripts/reparse_rebuild.py --db /path/to/tsb.db [--dry-run]
...
Motivation:
  ...
  full 20K-60K-char French narratives.  Both bugs are fixed in tsb_ingest
  >= this commit.  Running this script recovers ~2 209 previously-skipped
  rows on the mini-PC's existing tsb.db without re-downloading PDFs.
"""
import argparse
import sys
import os
...
from tsb_ingest import db, pipeline
from tsb_ingest.text import parse_event_title
...
def main():
    ...
    parser.add_argument("--db", required=True, help="Path to tsb.db SQLite file")
