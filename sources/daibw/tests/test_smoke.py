...
the stated grounds that "daibw_scraper.py runs its ingest at module scope, so
...
import daibw_scraper

HERE = pathlib.Path(__file__).resolve().parent.parent
SCRIPT = HERE / "daibw_scraper.py"
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert daibw_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (daibw_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "daibw" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(daibw_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == daibw_scraper.__name__]
