# aaicmv_ingest/cli.py
import argparse
import os

from . import httpc, aaicmv, db
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={
            "User-Agent": aaicmv.UA,
            "Referer": aaicmv.REFERER,
        },
        proxy=proxy,
        timeout=60,
        delay=aaicmv.DELAY,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="aaicmv-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aaicmv.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AAICMV_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AAICMV_PROXY env var)",
    )
