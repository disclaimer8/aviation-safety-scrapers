...
_VENDORED_MARKER = "VENDORED from _common/"
...
def test_no_sibling_keys_leak_in_source():
    ...
    Vendored copies are skipped. They are byte-identical to _common/, which
    the drift gate enforces, and their prose cites real sources by name as
    documentation — httpc.py's docstrings mention aaib and aaiu when
    explaining where the retry and TLS policy came from. That is not an
    identifier this package resolves, and it cannot route Maldives rows under
    Nepal's key, which is what this test exists to prevent. Editing the canon
    to satisfy a per-package guard would be the tail wagging the dog.
    """
    ...
    for py in PKG.glob("*.py"):
        ...
        if _VENDORED_MARKER in body:
            continue
        scanned.append(py.name)
        for m in pattern.finditer(body):
            offenders.append((py.name, m.group(1)))
    # A marker typo would silently empty the scan and make this pass vacuously.
    assert scanned, "no first-party modules scanned — is the vendored marker wrong?"
