...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=tsib.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="tsib-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover->fetch->build)",
    )
    ap.add_argument("--db", default="tsib.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--max-pages", type=int, default=None,
                    help="Cap listing pages walked (smoke runs)")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the listing is always "
                         "fully parsed from one fetch")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("TSIB_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $TSIB_PROXY env var)",
    )
    return ap
