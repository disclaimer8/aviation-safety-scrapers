...
def test_schema_tables_and_columns():
    ...
    rcols = {r["name"] for r in conn.execute("PRAGMA table_info(nbaai_reports)")}
    assert {
    ...
    acols = {r["name"] for r in conn.execute("PRAGMA table_info(nbaai_accidents)")}
    assert {
...
def test_country_default_ua():
    conn = db.connect(":memory:")
    db.init_schema(conn)
    conn.execute("INSERT INTO nbaai_accidents (case_id) VALUES ('NBAAI-UR-KTB-2019-10-21')")
    row = conn.execute(
        "SELECT country FROM nbaai_accidents WHERE case_id='NBAAI-UR-KTB-2019-10-21'"
    ).fetchone()
    assert row["country"] == "UA"
...
def test_status_index_exists():
    ...
    assert "idx_nbaai_reports_status" in indexes
