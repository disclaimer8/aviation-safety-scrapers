...
def test_discover_inserts_from_fixtures(make_client):
    ...
    # 80 was the old _NARRATIVE_FLOOR; this assertion is about the listing
    # producing a non-trivial narrative, not about publication admission.
...
def test_parse_html_only_tier_html():
    conn = _conn()
    body = "Катастрофа вертольота R-44. " * 40  # > _NARRATIVE_FLOOR, clean Ukrainian
    conn.execute(
...
def test_parse_mojibake_triggers_ocr(monkeypatch):
    ...
    recovered = ("РОЗСЛІДУВАННЯ КАТАСТРОФИ ЛІТАКА L-410. "
                 "Екіпаж загинув. Аеродром призначення. ") * 20  # usable Ukrainian
    ocr_calls = []
...
def test_parse_scanned_empty_triggers_ocr(monkeypatch):
    ...
    conn.commit()
    recovered = "Звіт про авіаційну подію. Пілот. Розслідування. Аеродром. " * 12
...
def test_parse_ocr_fails_falls_back_to_html(monkeypatch):
    conn = _conn()
    html_body = "Катастрофа літака в районі аеродрому. Розслідування комісії НБРТ. " * 12
    conn.execute(
        "INSERT INTO nbaai_reports (case_id, pdf_path, narrative_text, status) "
...
def test_build_projects_and_skips_thin():
    conn = _conn()
    good = "Розслідування катастрофи літака. " * 15
    conn.execute(
