...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="nbaai-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="nbaai.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("NBAAI_PROXY"),
        help="HTTP/SOCKS proxy URL (or set NBAAI_PROXY env var)",
    )
