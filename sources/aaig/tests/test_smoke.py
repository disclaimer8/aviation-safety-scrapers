...
the stated grounds that "aaig_scraper.py runs its ingest at module scope, so
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert aaig_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (aaig_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "aaig" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(aaig_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == aaig_scraper.__name__]
