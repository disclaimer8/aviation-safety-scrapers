...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={"User-Agent": caav.UA, "Referer": caav.REFERER},
        proxy=proxy, timeout=60, delay=caav.DELAY,
    )


def main(argv=None):
    ap = argparse.ArgumentParser(prog="caav-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="caav.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("CAAV_PROXY"),
        help="HTTP/SOCKS proxy URL (or set CAAV_PROXY env var)",
    )
