...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "X-2024-01-01", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident",
...
def test_build_skips_below_floor_scanned():
    ...
    conn = _conn()
    _seed_parsed(conn, "5Y-B-2024-01-01", narrative="X" * (pipeline._NARRATIVE_FLOOR - 1), event_class="Accident")
    assert pipeline.build(conn) == 0
    assert conn.execute("SELECT status FROM aaid_reports").fetchone()["status"] == db.STATUS_SKIPPED
...
def test_build_country_is_ke():
    conn = _conn()
    _seed_parsed(conn, "5Y-C-2024-01-01", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident")
    pipeline.build(conn)
