...
def make_client(proxy=None, **_kw):
    ...
    The pinned SSLContext goes through httpc's verify= argument rather than
    httpx's. httpx ignores verify= when a transport is supplied, and httpc
    always supplies one (retries, robots, SSRF) — so passing the context to
    httpx.Client here would silently drop the pin and fall back to the system
    store, which rejects this host's expired chain. The failure would look
    like a network error, not like a lost pin.
    """
    from . import httpc
    return httpc.make_client(
        headers=HEADERS, proxy=proxy, timeout=60, verify=make_ssl_context(),
    )
