# dgaccl_ingest/cli.py
import argparse
import os


from . import db, httpc, dgaccl
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=dgaccl.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="dgaccl-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover->fetch->build)",
    )
    ap.add_argument("--db", default="dgaccl.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the 7 year pages are "
                         "always walked in full")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("DGACCL_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $DGACCL_PROXY env var)",
    )
    return ap
