# aaicnp_ingest/cli.py
import argparse
import os

from . import httpc, aaicnp, db
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={
            "User-Agent": aaicnp.UA,
            "Referer": aaicnp.REFERER,
        },
        proxy=proxy,
        timeout=60,
        delay=aaicnp.DELAY,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="aaicnp-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aaicnp.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AAICNP_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AAICNP_PROXY env var)",
    )
