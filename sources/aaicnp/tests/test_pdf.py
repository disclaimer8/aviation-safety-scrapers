...
def test_extract_text_returns_stdout(monkeypatch):
    monkeypatch.setattr(pdf.subprocess, "run", lambda *a, **k: _Done(0, b"  Final report text  "))
    assert pdf.extract_text("x.pdf") == "Final report text"
...
def test_extract_text_none_path():
