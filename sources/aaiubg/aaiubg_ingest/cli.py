# aaiubg_ingest/cli.py
import argparse
import os

from . import httpc, aaiubg, db
...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(
        headers={
            "User-Agent": aaiubg.UA,
            "Referer": aaiubg.REFERER,
        },
        proxy=proxy,
        timeout=60,
        delay=aaiubg.DELAY,
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="aaiubg-ingest")
    ap.add_argument("mode", choices=["discover", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aaiubg.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AAIUBG_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AAIUBG_PROXY env var)",
    )
