...
def test_build_source_url_falls_back_to_report_url():
    conn = _conn()
    _seed_parsed(conn, "bg-x", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident",
...
def test_build_skips_below_narrative_floor():
    conn = _conn()
    _seed_parsed(conn, "LZ-Y_2020-01-01", narrative="X" * (pipeline._NARRATIVE_FLOOR - 1), event_class="Serious incident")
    assert pipeline.build(conn) == 0
    assert conn.execute(
...
def test_build_country_is_bg():
    conn = _conn()
    _seed_parsed(conn, "LZ-Z_2020-01-01", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Accident")
    pipeline.build(conn)
    assert conn.execute(
...
def test_build_report_type_from_event_class():
    conn = _conn()
    _seed_parsed(conn, "D-ASXP_2018-08-12", narrative="N" * (pipeline._NARRATIVE_FLOOR + 100), event_class="Serious incident")
    pipeline.build(conn)
    assert conn.execute(
