...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=rnsa.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="rnsa-ingest")
    ap.add_argument(
        "mode",
        choices=["discover", "fetch", "build", "all"],
        help="Pipeline stage to run (all = discover->fetch->build)",
    )
    ap.add_argument("--db", default="rnsa.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true",
                    help="Accepted for API parity; the per-year archive pages "
                         "are always walked in full")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("RNSA_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $RNSA_PROXY env var)",
    )
    return ap
