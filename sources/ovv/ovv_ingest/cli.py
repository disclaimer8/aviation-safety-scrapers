...
def _make_client(proxy=None, **_kw):
    ...
    return httpc.make_client(headers=ovv.HEADERS, proxy=proxy, timeout=120)


def _build_argparser():
    ap = argparse.ArgumentParser(prog="ovv-ingest")
    ap.add_argument(
        "mode",
    ...
    ap.add_argument(
        "--proxy",
        default=os.environ.get("OVV_PROXY"),
        help="SOCKS5/HTTP proxy URL (default: $OVV_PROXY env var)",
    )
    return ap
