# aicpng_ingest/cli.py
import argparse
import os

from . import httpc, aicpng, db
...
def _make_client(proxy=None, **_kw):
    ...
    # cookies ride through **kwargs to httpx.Client; the Drupal front end
    # needs them to serve the AJAX payloads this source parses.
    return httpc.make_client(
        headers=aicpng.HEADERS,
        proxy=proxy, timeout=60, delay=aicpng.DELAY,
        cookies=aicpng.COOKIES,
    )


def main(argv=None):
    ap = argparse.ArgumentParser(prog="aicpng-ingest")
    ap.add_argument("mode", choices=["discover", "refetch", "fetch", "parse", "build", "all"])
    ap.add_argument("--db", default="aicpng.db")
    ap.add_argument("--pdf-dir", default="pdfs")
    ap.add_argument("--full", action="store_true")
    ap.add_argument(
        "--proxy",
        default=os.environ.get("AICPNG_PROXY"),
        help="HTTP/SOCKS proxy URL (or set AICPNG_PROXY env var)",
    )
