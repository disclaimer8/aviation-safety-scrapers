...
the stated grounds that "sibpk_scraper.py runs its ingest at module scope, so
...
import sibpk_scraper

HERE = pathlib.Path(__file__).resolve().parent.parent
SCRIPT = HERE / "sibpk_scraper.py"
...
def test_the_module_imports_without_touching_the_network():
    ...
    assert sibpk_scraper.__file__


def test_the_header_still_records_where_the_data_comes_from():
    doc = (sibpk_scraper.__doc__ or "").strip()
    assert len(doc) >= 60, (
    ...
    lowered = doc.lower()
    assert "sibpk" in lowered or "http" in lowered, (
...
def test_it_exposes_a_callable_entry_point():
    functions = [name for name, obj in vars(sibpk_scraper).items()
                 if inspect.isfunction(obj) and obj.__module__ == sibpk_scraper.__name__]
