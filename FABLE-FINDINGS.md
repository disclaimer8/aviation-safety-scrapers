<!-- Point-in-time notes, not a live document. -->

# Source leads from the Fable review, rescued from the session transcript

Recovered 2026-09-11. These were produced by a review agent and existed only
in a conversation that had already been compacted once.

**Most of this is now done.** Hong Kong, Guyana and Belize were promoted into
`sources/` on 2026-09-11 and 09-12, along with 27 other archived scrapers.
What remains from these notes is Eswatini and São Tomé — real archives of
roughly 8 and 7 documents each, with no scraper written.

Treat the numbers below as an agent's report, not as measurements. Where they
were checked against the sources they were directionally right and
consistently high; the parser-level figures in the commit history are the ones
to trust.

---

NOT yet verified end-to-end. Spot-checked live 2026-09-11:
Hong Kong 200 / 176 pdf links, Guyana 200 / 29, Eswatini 200 / 11, Belize 200 (links are detail pages, not direct PDFs).
...
<task-notification>
<task-id>a5fdd40ae929ab062</task-id>
<tool-use-id>toolu_01SaR9RT87YHEcNA8g5HzgZo</tool-use-id>
<output-file>/private/tmp/claude-501/-Users-denyskolomiiets/6fea7b88-af9e-459d-8d37-3c78a6c01b27/tasks/a5fdd40ae929ab062.output</output-file>
<status>completed</status>
<summary>Agent "Find sources for unassessed countries" finished</summary>
<note>A task-notification fires each time this agent stops with no live background children of its own. The user can send it another message and resume it, so the same task-id may notify more than once.</note>
<result>All research is done; nothing further to fetch. Final report follows.

# Coverage assessment — 57 `coverage_status='unknown'` territories

**Coverage: 57 of 57 checked.** Every URL below was fetched by me (curl/WebFetch) and the status code is the one I received on 2026-09-11. 52 resolved, 5 UNRESOLVED (ER, LS, MU, NU, PS).

**Five of the 57 are mis-flagged — they already have scrapers in this repo:** AR (`sources/jst`), BR (`sources/cenipa`), SG (`sources/tsib`), MO (`sources/aacm`), SC (`sources/scaa`). Only AR/BR were named in the brief; SG/MO/SC I found in `registry.yaml`.
...
**AR Argentina** — OWN_ARCHIVE. Authority: Junta de Seguridad en el Transporte (JST). URL (from `sources/jst/jst_ingest/jst.py`, `MANIFEST_URL`): `https://so.jst.gob.ar/static/informes/Index.json`; PDFs at `https://so.jst.gob.ar/static/informes/{path}`. http 200. Documents: 3,268 report entries across 1,990 expedientes (I parsed the manifest). Format PDF. Evidence: the manifest is a dict keyed by 8-digit expediente → `[{tipo, path}]`.
...
**MU Mauritius** — Department of Civil Aviation site `https://civil-aviation.govmu.org/` http 200 (the `civilaviation.` host does not resolve) has no safety/investigation section; investigation authority not identified.
**NU Niue** — no Niue authority page found; the NZ CAA agreements page (200) contains no "Niue" text server-side. Not determined.
**PS Palestine** — Ministry of Transport page for the Civil Aviation Authority refused connection (000); Gaza airport closed since 2001. Not determined.
...
5. Do not spend time on KM/DJ/GW/SS/CK/SC/MO — nothing to scrape live; SC and MO are already handled (0 by design / Wayback).</result>
<usage><subagent_tokens>218743</subagent_tokens><tool_uses>116</tool_uses><duration_ms>1130254</duration_ms></usage>
</task-notification>
</system-reminder>
...
5 candidate lines
  **HK Hong Kong** — Authority: Air Accident Investigation Authority (AAIA), Transport and Logistics Bureau. URL: `https://www.tlb.gov.hk/aaia/eng/investigation_reports/index.html` http 200. Documents: 
  **GY Guyana** — Authority: Guyana Civil Aviation Authority, Accident Investigation Department (AAID; older reports headed GAAIIU). URL: `https://www.gcaa-gy.org/AAID.html` http 200. Documents: 29, all
  **BZ Belize** — Authority: Belize Department of Civil Aviation, Accident Investigation Unit (AIU). URL: `https://www.civilaviation.gov.bz/index.php/accident-investigation-unit-aiu/accident-reports` ht
  **SZ Eswatini** — Authority: Eswatini Civil Aviation Authority, Aircraft Accident and Incident Investigations Department (AAIID). URL: `https://www.eswacaa.co.sz/aaiid/` http 200. Documents: 8 (final 
  Three Fable agents completed. Highlights: `eaaid` has remote shell injection via `ssh HETZNER "curl ... '{url}'"` with `[^"]+` capture; 13 archive packages with silent truncation; 31/31 without respon
